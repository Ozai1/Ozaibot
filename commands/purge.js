        ...
        if (args[1]) {
            args.forEach(async arg => {
                if (arg.toLowerCase() === 'silent') {
                    ...
                    let member2 = await GetMember(message, arg, Discord, false, false)
                    if (member2) {
            ...
            if (amount > 100) {
                return ExecuteLargeBulkDeleteWithOptions(message, amount, conformationmessage, members, silent, haslinks, hasinvites, hasbots, hasembeds, hasfiles, hasusers, hasimages, hasmentions, haspins)
            } else {
                return ExecuteBulkDeleteWithOptions(message, amount, conformationmessage, members, silent, haslinks, hasinvites, hasbots, hasembeds, hasfiles, hasusers, hasimages, hasmentions, haspins)
            }
        } else {
...
async function ExecuteBulkDeleteWithOptions(message, amount, conformationmessage, members, HasSilent, HasLinks, HasInvites, HasBots, HasEmbeds, HasFiles, HasUsers, HasImages, HasMentions, Includepins) {
    let amountcached = 0;
    ...
    await message.channel.messages.fetch(options).then(messages => {
        ...
        messages.forEach(async (message2) => {
            if (message2.pinned) {
                ...
                if (HasImages) {
                    let imagefoundinattatch = false
                    if (message2.attachments.length) {
                        console.log('attatchments found')
                        message2.attatchments.forEach(attachments => {
                            console.log(`${attachments}.`)
                            if (imagefoundinattatch === false) {
                                if (attachments.contentType.startsWith('image')) {
                                    imagefoundinattatch = true
                                    let messagetime = (`${Number(message2.id / 4194304 + imissjansomuchithurts)}`).slice(0, -7) // what the time of the message was,cut off all decimal places so we are at seconds
                                    if (messagetime.length > 10) {
                                        messagetime = messagetime.slice(0, -1) // if the message time has 1 extra decimal place, cut it the fuck off
                                    }
...
async function ExecuteLargeBulkDeleteWithOptions(message, amount, conformationmessage, members, HasSilent, HasLinks, HasInvites, HasBots, HasEmbeds, HasFiles, HasUsers, HasImages, HasMentions, Includepins) {
